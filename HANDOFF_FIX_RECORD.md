...
- 日期：2026-09-19
- 参考：`每日1关弹窗.png`（美术）+ `Screenshot_20260919_125837.jpg`（友商成品效果）

## 一、怎么量的槽位坐标

弹窗是一整张烤好的美术，代码只往几块留白里填数字。问题是「留白在哪」不好口算，
所以用了个土办法：把美术裁掉透明边（原图 941x1672，实际画面是 x7-934 / y197-1455），
缩到一半，再用 sharp 叠一层**带百分比刻度的网格 SVG**，直接在图上读坐标。
量完写进 `modals.js` 的 `DAILY_SLOTS`（相对美术自身宽高的百分比，换美术要重量）：

| 槽位 | x | y | w | h |
|---|---|---|---|---|
| ✕ 关闭 | 0.845 | 0.05 | 0.12 | 0.11 |
| 挑战人数 | 0.32 | 0.155 | 0.17 | 0.07 |
| 通关人数 | 0.785 | 0.155 | 0.17 | 0.07 |
| 剩余次数 | 0.565 | 0.755 | 0.165 | 0.062 |
| 前往挑战 | 0.21 | 0.822 | 0.58 | 0.10 |

美术 1.4MB 的 PNG 直接进包会把主包顶到 4MB 上限，裁掉透明边后转 WebP（q86）压到 157K：

```bash
node -e "require('sharp')('mp-tools/art-src/daily_level_master.png')
  .extract({left:7,top:197,width:928,height:1259})
  .webp({quality:86,alphaQuality:90}).toFile('majiang-game/static/ui/daily_level.webp')"
```

## 二、次数为什么按本地日期算

跨 00:00 重置这件事，靠手点根本验不出来（要么等一天，要么改系统时间），
而写错的后果是「第二天点进去次数没回来」或者「同一天能无限刷」。
所以 `src/daily.js` 把这段写成了不碰 wx / 不碰网络的纯函数，
`mp-tools/daily-check.cjs` 直接喂日期断言：跨天补、同天不补、老存档缺字段自愈、
后台把次数调小当天就封顶、扣不动时不会扣成负数。

用**本地日期**而不是服务器时间，是因为这游戏没有账号体系、存档就在本机；
拿服务器时间反而会出现「玩家改了手机时区、次数忽然回不来」。代价是改系统时间能多刷几次
——单机休闲游戏，认了。

## 三、奖励数值和美术是耦合的

弹窗上「3倍」「x200」是**烤在图里**的，而数值现在从后台下发。两边一旦不一致，
玩家看到的和拿到的就不是一回事。所以钉了三道：

- 后台保存时，若数值与美术不符，toast 会直接说「记得换美术」；
- `daily-check.cjs` 断言客户端兜底值、后台默认值、美术上印的三者一致；
- `admin-check.cjs` 断言客户端兜底值 == 后台默认值（拉不到后台时两边也得对得上）。

结算走 `store.js` 的 `dailyChallengeBonus()`：基础结算仍按 1 倍发，这里只补
(倍数-1) 的星星差额 + 那笔额外金币，避免把「匹配数」任务进度也一起放大 3 倍。

## 四、顺手做的一次抽取

广告和每日一关都要「从同一个后台拉配置、往同一个接口上报、拉不到就降级」，
于是把这套从 `ads.js` 抽成了 `src/backend.js`（地址解析、超时兜底、缓存、
非预期响应打状态码）。抽完 `ads-check.cjs` 9 节全绿，说明行为没变。

## 五、红点，而不是自动弹窗

产品口径确认下来了：**不自动弹**。改成首页四个入口在「有事可做」时点红点，做完就灭——
每日挑战看今天还有没有次数、每日任务看有没有达成未领的、添加桌面看那 200 金币领没领、
主题装扮看点开过没有（这是唯一为红点新增的存档字段 `badgesSeen.theme`，
其余三个都能从既有状态推出来，不必另记）。

判断集中在 `home.js` 的 `needsBadge()`。这里唯一的纪律是别图省事写死 `true`：
红点永远亮着等于没有红点。

## 六、还没做的
- 挑战人数 / 通关人数 是运营手填的展示数字，不是真实统计。真要做成真实数据，
  得在 `/api/stats` 里按日聚合，客户端上报一次「开始挑战 / 通关」。
