 ...
 * CommonJS 的循环 require 会让先被引的一方拿到半成品 exports（`screen` 是 undefined），
...
function initScreen() {
  ...
  screen.safeTop = Math.max(statusBar, safe ? safe.top * dpr : 0, screen.H * 0.04);
  screen.safeBottom = safe ? Math.max(0, (info.windowHeight - safe.bottom) * dpr) : 0;
  return screen;
}

module.exports = { screen, initScreen };
