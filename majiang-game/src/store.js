 ...
 *
 * 这是 majiang-mp/src/game/state.js 的小游戏版。除了 uni.* → wx.*，只改了一处：
 * 原文件用 vue 的 `reactive` 包状态、用 `watch(..., { deep: true })` 自动落盘。
 * 小游戏里没有 vue，但「改哪个字段都会自动存」这个语义必须留着——全项目有几十处
 * 直接写 gameState.xxx 的地方，逐个补 save() 调用迟早漏掉一处，存档就开始丢数据。
 *
 * 所以换成一个几十行的深度 Proxy 观察器：任何一层的写入都会标脏，
 * 然后合并到下一个 tick 落盘（避免一次结算里连写十几个字段就写十几次 storage）。
 * 它同时维护一个 revision 计数，场景层可以用它判断「状态变了，这帧要重画」。
 */

/** 深度 Proxy：对象树里任何一处赋值/删除都会回调 onChange。 */
...
/* 状态版本号。每次状态有写入就 +1，场景层比对它来决定是否需要重绘，
 * 免得每帧都把整个首页重画一遍。 */
...
/* 标脏 + 合并落盘。50ms 的合并窗口足够把一次结算里的十几次写入并成一次 storage 写。 */
...
const gameState = observe(loadStored(), onStateChanged);

/** 立刻落盘。小游戏被切后台（wx.onHide）时要主动调一次，别等合并窗口。 */
function saveNow() {
  try {
    wx.setStorageSync(STORAGE_KEY, JSON.parse(JSON.stringify(gameState)));
  } catch (e) {}
}
...
function consumeStamina(count = 1) {
...
function refundStamina(count = 1) {
...
// 格式化秒数为 hh:mm:ss
function formatLongSeconds(sec) {
  if (!sec || sec < 0) sec = 0;
 ...
 * 保证默认行为与旧版本完全一致。 */
function winLevelAction(level, matchTriples = 0, multiplier = 1) {
