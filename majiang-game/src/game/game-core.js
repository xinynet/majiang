...
const SYMBOLS = ['一筒', '二筒', '三筒', '四筒', '五筒', '六筒', '七筒', '八筒', '九筒',
...
function tileCountForLevel(level, config) {
 ...
 */
function makeLayout(level, rnd, aspect = 0.76, config) {
 ...
 */
function boardMetrics(tiles, level, viewW, viewH) {
...
function createGame({ motion, emit = () => {}, reducedMotion = false } = {}) {
  const state = {
...
module.exports = { faceKey, seeded, tileCountForLevel, makeLayout, boardMetrics, createGame, SYMBOLS, PICTURE_TILES, SLOTS, ROUND_SECONDS, TILES_ACROSS, MAX_LAYERS };
