...
四个入口在「有事可做」时右上角点红点，做完就灭。**没有自动弹窗**——弹窗只在点入口时出现。

| 入口 | 亮红点的条件 |
|---|---|
| 每日挑战 | 今天还没挑战过（`daily.attemptsLeft() > 0`） |
| 每日任务 | 有任务已达成但没领（每日/长线一起看） |
| 添加桌面 | 那份 200 金币奖励还没领（`desktopRewardClaimed`） |
| 主题装扮 | 还没点开看过（`badgesSeen.theme`，存档里唯一为红点存在的字段） |

判断集中在 `home.js` 的 `needsBadge()`。**别在里面写死 `true`**：红点永远亮着，
玩家很快就学会无视它，这四个入口就白挂了。

## 牌桌背景的几何

野餐布（绿地 + 蓝白格）那条带子**按屏宽定高**：`viewport.W * 200/750`，锚在顶部拉伸铺满
（等价小程序 `.g-meadow` 的 `width:100%` + `height:200rpx` + `mode="scaleToFill"`）。
实测蓝白格恒定落在屏幕顶部 8.5%~12.3%，和小程序逐像素对齐，状态行「关卡/种类」在它下面。

别改回按**屏高**取高度或用 `drawCover`：那样裁切量随机型比例变化，蓝白格会上下漂——
预览机型上漂到 22%，开发者工具的机型上漂到 27%，直接跑到状态行下面去。
